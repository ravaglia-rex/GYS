/**
 * Bank markdown ships relative `assets/item_….svg` paths. Those resolve against
 * the current page URL, so item bank (`/platform-admin/item-bank`) and a live
 * exam (`/assessments/…/tier/1/take`) fetch different — usually 404 — files.
...
export function resolveExamFigureSrc(src: string | undefined | null): string {
  ...
  if (/^item_[^/]+\.svg$/i.test(file)) return `${AR_L1_DIR}${file}`;
  return raw;
}

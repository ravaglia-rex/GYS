...
} from "../../db/passwordResetCollection";

interface CustomTokenPasswordFormProps {
  token: string;
}
...
const CustomTokenPasswordForm: React.FC<CustomTokenPasswordFormProps> = ({ token }) => {
  ...
  useEffect(() => {
    ...
    validatePasswordResetToken(token)
      ...
      .catch(() => {
        if (!cancelled) setTokenValid("invalid");
      });
    return () => {
      cancelled = true;
    };
  ...
  const onSubmit = async (data: { password: string; confirm_password: string }) => {
    try {
      ...
      await completePasswordResetWithToken(token, data.password);

      const { user } = await signInWithEmailAndPassword(auth, email, data.password);
      try {
        if (isPlatformAdmin) {
          const authToken = await user.getIdToken();
          const { default: authTokenHandler } = await import(
            "../../functions/auth_token/auth_token_handler"
          );
          authTokenHandler.setAuthToken(authToken);
          const { verifyPlatformAdminPasswordSetup } = await import(
            "../../db/platformAdminCollection"
          );
          ...
          authTokenHandler.setAuthToken(authToken);
          const { checkSchoolEmail, verifySchoolEmail } = await import(
            "../../db/schoolAdminCollection"
          );
        ...
        toast({
          ...
          description: err.response?.data?.error ?? err.message ?? "Could not finish account setup.",
        });
      }
      ...
      Sentry.withScope((scope) => {
        scope.setTag("location", "CustomTokenPasswordForm.complete");
  ...
  if (tokenValid === "checking") {
    return (
      <div className={`${cardClass} flex justify-center py-12`}>
        <Spinner />
      </div>
    );
  }
    ...
    return (
      <div className={cardClass}>
        <p className="text-center text-slate-200">
          This password link is invalid or has expired. Request a new one from the sign-in page via
          Forgot password.
        </p>
        <div className="mt-6 flex justify-center">
          <Button
            ...
            className="h-11 bg-gradient-to-r from-indigo-600 to-blue-600"
          >
            Back to sign in
          </Button>
        </div>
...
export default CustomTokenPasswordForm;
